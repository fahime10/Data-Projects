...
ggplot(returns_df, aes(x = Stock, y = Return, fill = Sector)) +
  geom_boxplot() +
  labs(title = "Box plot of Daily Returns Distribution by Stock") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

ggplot(returns_df, aes(x = Stock, y = Return, fill = Sector)) +
  geom_violin() +
  labs(title = "Violin plot of Daily Returns Distribution by Stock") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
View(summary_table_sorted_by_volatility)
...
rolling_avg <- rolling_avg %>% 
  mutate(Stock = dplyr::recode(Stock, "XLK" = "Tech", "XLE" = "Energy", 
                        "XLV" = "Healthcare", "XLF" = "Finance"))
...
# Examine events like COVID and the 2008 market crash
...
ggplot(covid_growth, aes(x = date, y = Growth, color = Sector)) +
  ...
  annotate("text", x = as.Date("2020-03-01"), 
           y = max(covid_growth$Growth, na.rm = TRUE),
           label = "COVID Crash", color = "red", angle = 0, vjust = 1.5, 
           hjust = 1.3,
           size = 3) +
...
ggplot(corr_df, aes(Var1, Var2, fill = value)) +
  ...
  scale_fill_gradient2(low = "blue", 
                       ...
                       high = "red", 
                       midpoint = 0.5) +
  scale_x_discrete(labels = c("XLK" = "Tech", "XLE" = "Energy", 
                              "XLV" = "Healthcare", "XLF" = "Finance")) +
  scale_y_discrete(labels = c("XLK" = "Tech", "XLE" = "Energy", 
                               "XLV" = "Healthcare", "XLF" = "Finance")) +
